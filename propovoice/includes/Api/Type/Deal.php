...
class Deal {
    ...
    public function routes() {
        ...
        register_rest_route(
            'ndpv/v1', '/deals' . ndpv()->plain_route(), [
				'methods' => 'GET',
				'callback' => [ $this, 'get' ],
				'permission_callback' => [ $this, 'get_per' ],
			]
    ...
    public function deal_query( $param, $stage_id = null ) {
        ...
        while ( $query->have_posts() ) {
            ...
            $query_data['probability'] = isset( $query_meta['probability'] ) ? $query_meta['probability'][0] : '';
            //custom field
    ...
    public function get_single( $req ) {
        ...
        $query_data['budget'] = isset( $query_meta['budget'] )
            ? $query_meta['budget'][0]
            : '';
        $query_data['currency'] = isset( $query_meta['currency'] )
            ? $query_meta['currency'][0]
            : '';
        $query_data['probability'] = isset( $query_meta['probability'] )
            ? absint( $query_meta['probability'][0] )
            : '';
    ...
    public function create( $req ) {
        ...
        if ( $reg_errors->get_error_messages() ) {
            ...
            $data = [
                ...
                'menu_order'  => 0, //added default menu order for kanban board
            ];
            ...
            if ( ! is_wp_error( $post_id ) ) {
                ...
                if ( $lead_id ) {
                    $get_lead_person = get_post_meta( $lead_id, 'person_id', true );
                    if ( $get_lead_person ) {
                        $person_id = $get_lead_person;
                    }

                    $get_lead_org = get_post_meta( $lead_id, 'org_id', true );
                    if ( $get_lead_org ) {
                        $org_id = $get_lead_org;
                    }
                }
...
update_post_meta($lead_id, "_ndpv_allowed_users", $staffs);
                ...
                if ( ! empty( $param['lead_id'] ) ) {
                    do_action( 'ndpvp_webhook', 'lead_to_deal', $param );
                } else {
                    do_action( 'ndpvp_webhook', 'deal_add', $param );
                }
            ...
            if ( ! is_wp_error( $post_id ) ) {
                if ( $stage_id ) {
                    ...
                    if ( $previous_deal_stage[0]->term_id !== $stage_id ) {
                        do_action( 'ndpvp_webhook', 'deal_stage_change', $param );
                    }
                }
                ...
                if ( $reorder ) {
                    ...
                    do_action( 'ndpvp_webhook', 'deal_edit', $param );
                }
  ...
  public function reorder_posts($order = [])
  {
    ...
    public function delete( $req ) {
        ...
        do_action( 'ndpvp_webhook', 'deal_del', $ids );
